 ...
 * @file texture.hpp
 * @author Simon Tanev
 * @brief File containing Texture class
 */
 ...
 * @class Texture
 *
 * Holds information about and data of a texture
 */
class Texture {
public:
    ...
    /**
     * Loads the texture from a filename using lodepng library
     *
     * @param[in] filename      filename to load the texture from
     */
    explicit Texture(const char* filename) {
        data = nullptr;

        if (const unsigned int error = lodepng_decode32_file(&data, &width, &height, filename)) {
        ...
        data_size = 4 * width * (height - 1) + 4 * (width - 1) + 4;
    }

    /**
     * Frees the data stored in the texture
     */
    ~Texture() {
     ...
     */
    vec3 at(const unsigned int u, const unsigned int v) const {
        ...
        return vec3(clamp(r, 0.0f, 1.0f), clamp(g, 0.0f, 1.0f), clamp(b, 0.0f, 1.0f));
    }
     ...
     */
    vec3 bilinear_sample_xy(const float u, const float v, const unsigned int u1, const unsigned int u2, const unsigned int v1,const unsigned int v2 ) const {
        const vec3 top_left = at(u1, v2);
        const vec3 top_right = at(u2, v2);
        const vec3 bottom_left = at(u1,v1);
        const vec3 bottom_right = at(u2, v1);
        ...
        return vec3(top_left * top_left_mult + top_right * top_right_mult + bottom_left * bottom_left_mult + bottom_right * bottom_right_mult);
    }
     ...
     */
    vec3 bilinear_sample_x(const float u, const unsigned int u1, const unsigned int u2, const unsigned int v) const {
        const vec3 right = at(u2, v);
        const vec3 left = at(u1, v);

        const float right_mult = static_cast<float>(u2) - u;
        const float left_mult = u - static_cast<float>(u1);

        return vec3(right * right_mult + left * left_mult);
    }
     ...
     */
    vec3 bilinear_sample_y(const unsigned int u, const float v, const unsigned int v1, const unsigned int v2) const {
        const vec3 top = at(u, v2);
        const vec3 bottom = at(u, v1);

        const float top_mult = static_cast<float>(v2) - v;
        const float bottom_mult = v - static_cast<float>(v1);

        return vec3(top * top_mult + bottom * bottom_mult);
    }
     ...
     */
    vec3 sample_uv(const vec2 tex_coord) const {
        const vec2 uv = tex_coord % 1.0f;
        const float u = uv.x * static_cast<float>(width-1);
        const float v = uv.y * static_cast<float>(height-1);
     ...
     * @param[in] tex_coord     input uv coordinates
     */
    vec3 sample_uv_normal(const vec2 tex_coord) const {
        const vec2 uv = tex_coord % 1.0f;
        const float u = uv.x * static_cast<float>(width-1);
